...
const renderizarKPIs = (listaUsinas) => {
    ...
    listaUsinas.forEach(u => {
        ...
        armazTotal += Number(u.total_kwh_armazenado) || Number(u.capacidade_util_total_kwh) || 0;
    });
    ...
    if (elTotalUsinas) elTotalUsinas.textContent = listaUsinas.length;
    ...
    usinas.forEach(usina => {
        ...
        if (usina.baterias && usina.baterias.length > 0) {
            bateriasHtml = usina.baterias.map(b => {
                const kwhBateria = b.capacidade_util_kwh || b.energia_util_kwh || 0;
                return `
                <div class="bateria-item" style="margin-bottom: 6px; padding: 6px 10px; background: rgba(30, 41, 59, 0.7); border-radius: 6px; border: 1px solid #334155; display: flex; justify-content: space-between; align-items: center; gap: 8px;">
                    ...
                    <div style="display: flex; gap: 4px; flex-shrink: 0;">
                        <button class="btn-sm btn-warning" style="padding: 2px 6px; font-size: 0.75rem;" onclick="abrirEdicaoBateria(${b.id}, ${usina.id}, '${b.tecnologia}', ${b.capacidade_ah}, ${b.tensao_nominal_v}, ${b.dod_percentual}, ${b.quantidade})" title="Editar esta bateria">✏️ Editar</button>
        ...
        const totalKwhUsina = usina.total_kwh_armazenado || usina.capacidade_util_total_kwh || 0;
        ...
        tr.innerHTML = `
            <td>
                ...
                <small style="color: #94a3b8; margin-left: 2px;">${usina.cidade || ''}</small>
            </td>
            ...
            <td>
                <div style="display: flex; gap: 4px; flex-wrap: wrap;">
                    <button class="btn-sm btn-primary" onclick="abrirNovaBateriaModal(${usina.id})">+ Bateria</button>
                    <button class="btn-sm btn-secondary" onclick="prepararEdicaoUsina(${usina.id}, '${usina.nome}', ${usina.potencia_kwp}, ${usina.tensao_sistema_v}, '${usina.tipo_sistema}', '${usina.cidade || ''}', '${usina.data_instalacao || ''}')">✏️ Editar Usina</button>
